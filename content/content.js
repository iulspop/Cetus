...
window.addEventListener("cetusMsgIn", function(msg) {
    try {
        ...
        // Extension context invalidated (e.g. extension reloaded while page still open)
    }
}, false);
...
const dispatchCetusData = function(data) {
    const evt = new CustomEvent("cetusData", { detail: JSON.stringify(data) });
    window.dispatchEvent(evt);
};
...
loadOptions(function(extensionOptions) {
    const cetusData = { cetusOptions: extensionOptions };
        ...
        if (result !== null) {
            ...
            if (typeof savedPatches !== "undefined") {
                ...
                if (injectPatches.length > 0 || preinstantiateCallbacks.length > 0) {
                    ...
                    cetusData.cetusPatches = injectPatches;
                    cetusData.cetusCallbacks = allCallbacks;
                }
            }
        }

        dispatchCetusData(cetusData);
    });
});
